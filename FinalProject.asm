ORG &H000
JUMP Init  ; Begin program at x000
;***************************************************************
;* Initialization
;***************************************************************
Init:
...
Main: ; "Real" program starts here.
	...
	OUT LCD
	JUMP   CalcAngle
	;The point of this sample is to get the robot to a predetermined coordinate
;This code will attempt to get the robot from 0,0 to 500,500 (if there are total almost 1000 coordinates)
;current theory: get the desired point and current point and read in a loop
;first: turn while stopped to face target
;then proceed at medium speed, read angle and increment speeds until angle is correct
...
CalcAngle: 
	...
	STORE dY
	STORE AtanY
	;dX and dY now have updated values, take the tangent of Y/X to get the angle to the point
    CALL Atan2 ;function provided, uses variables AtanX and AtanY, stores 0-365 angle in AC
	STORE ThetaGoal
	IN THETA
	STORE NowTheta
	SUB ThetaGoal
	OUT SSEG2
	OUT LCD
	CALL   Mod360      ; remove negative numbers ; current in the AC is the diff between theta and desired theta
	SUB    Deg180        ; test which semicircle error is in
...
NeedRight:
	LOAD   NowTheta
	SUB    ThetaGoal    ; get the turn error
	CALL   Mod360      ; fix errors around 0
	SUB    DeadZone
	JNEG   NoTurn      ; stop moving if close
	ADD    DeadZone
...
goForward: 
	...
	;Waits so I can tell that it turned first
    LOAD FFast
	STORE RSpeed
	STORE LSpeed
...
Loop:
    ...
    CALL CalcError ;error includes distance and angle
	...
	CALL   Mod360      ; remove negative numbers ; current in the AC is the diff between theta and desired theta
	SUB    Deg180        ; test which semicircle error is in
	JPOS   Left    ; >180 means need left turn
	JNEG   Right   ; otherwise, need right turn
	JUMP   Loop    ;in this case, no turning
Left:
	LOAD LSpeed
	CALL Abs
	SUB  FSlow
...
;this begins the loop where the robot will continually adjust until reaching the desired point
